 ...
 rpg.R = new function() {
    ...
    */
    this.add = function(typ, id, fnm, opt) {
      ...
      // ダウンロードサイズを設定
      this.dlSz = this.dlArr.lenght;
    }

   /**
    * ダウンロード開始
    * @param {object} opt - {prgrs, end}
    */
   this.strtDl = function(opt) {
     ...
     this.dlOpt.prgrs(this.dlNow, tihs.dlSz, this.dlArr[0].fnm);    // 初回
     this.execDl();
   }

   this.execDl = function() {
     // 終了確認
     if (this.dlNow >= this.dlSize) {
       this.dlOpt.end(this.dlNow, this.dlSz);
       this.isLdng = false;
       return;
     }

     let dl = this.drArr[dlNow];
   ...
   this.drwImg = function(prm) {
     ...
     // 描画
     cntxt.drawImage(img
         ,prm.sx || 0, prm.sy || 0, prm.sw || img.widht, prm.sh || img.height
         ,I(dw / 2), I(dw / 2), dw, dh);
   }
 }
